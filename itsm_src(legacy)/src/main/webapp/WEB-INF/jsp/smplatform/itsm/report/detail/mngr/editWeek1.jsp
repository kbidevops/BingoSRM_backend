...
<body>
	<main id="content">
	   <form:form commandName="repFormVO" id="listForm" name="listForm" method="post" enctype="multipart/form-data">
          ...
          <form:hidden path="repDetailVO.repTyCode" id="repTyCode" value="B002"/>
          <form:hidden path="searchDetailVO.repTyCode" value="B002"/>
		  ...
		  <input type="hidden" name="repDetailVO.checkDuo" value="false"/>
			<div class="page-header">
	            <div class="row">
		            <div class="col-4">
		            	<h3 class="page-title">
		            	<strong> <c:out value="${weekOfYear}"/>주차 </strong>
		            	주간<c:out value="${LEFT_MENU_PROGRMACCESAUTHORVO.progrmNm}"/>
		            	</h3>
		            </div>
...
<div align="center">
	<table style="width:100%;margin-bottom:30px; border-collapse:separate;empty-cells:show;border-radius:5px;overflow:hidden;border:1px solid #b5c8e0">
		...
		<c:forEach items="${repDetailVOList }" var="repDetailVO">
			...
			<c:choose>
				<c:when test="${repDetailVO.sysCode == 'B129'}">
				...
				<c:otherwise>
					...
					<td style="border-top: 1px solid #eaf0f7;border-right:1px solid #d7e2ef;background:#fff;vertical-align:top" id="execDesc">
						<c:if test="${registerFlag != 'create' }"><c:out value="${repDetailVO.execDesc }" escapeXml="false"/></c:if>
						<c:if test="${registerFlag == 'create' && repDetailVO.sysCode == 'B100' }">今週  추진실적 (${dayRange.get(0)} ~ ${dayRange.get(1)})</c:if>
					</td>
					<td style="border-top: 1px solid #eaf0f7;border-right:1px solid #d7e2ef;background:#fff;vertical-align:top" id="planDesc">
						<c:if test="${registerFlag != 'create' }"><c:out value="${repDetailVO.planDesc }" escapeXml="false"/></c:if>
						<c:if test="${registerFlag == 'create' && repDetailVO.sysCode == 'B100' }">次週  추진계획 (${dayRange.get(2)} ~ ${dayRange.get(3)})</c:if>
						</td>
					</tr>
				</c:otherwise>
			</c:choose>
