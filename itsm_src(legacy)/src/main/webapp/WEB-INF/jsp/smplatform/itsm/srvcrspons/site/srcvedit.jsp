...
*/
    ...
    function fncSave() {
      ...
      if (document.listForm.elements['srvcRsponsVO.processTerm'].value === '') {
          alert('SR처리시간을 입력하세요.');
          return;
      }

      if (document.listForm.elements['srvcRsponsVO.rspons1stDtDateDisplay'].value === '') {
          alert('SR접수시간을 입력하세요.');
          return;
        }

      if (document.listForm.elements['srvcRsponsVO.rspons1stDtTimeDisplay'].value === '') {
          alert('SR접수시간을 입력하세요.');
          return;
        }
...
*/
      ...
      if (requstAtchmnflFileCnt > 0 && rsponsAtchmnflFileCnt > 0) {
        ...
        frm.action = "<c:url value="${registerFlag == 'create' ? '/srvcrspons/site/updatesrcv.do' : '/srvcrspons/site/updatesrcv.do'}"/>";
        frm.submit();
      }
    }
    ...
    function fncRetrieveList() {
      ...
      document.listForm.action = "<c:url value='/srvcrspons/site/retrieveSrRcvList.do'/>";
      </c:if>
      <c:if test="${!empty srvcRsponsFormVO.returnListMode}">
      document.listForm.action = "<c:url value='/srvcrspons/site/retrieveSrRcvList.do'/>";
      </c:if>
...
<main id="content">
  ...
  <div class="page-header">
    <h3 class="page-title">
      <c:out value="${LEFT_MENU_PROGRMACCESAUTHORVO.progrmNm}"/> <c:out value="${registerFlag == 'create' ? '' : ''}"/>
    </h3>
    <div class="btnbox fr">
...
-->
  ...
  <form:form commandName="srvcRsponsFormVO" id="listForm" name="listForm" method="post" enctype="multipart/form-data">
    ...
    <form:hidden path="srvcRsponsVO.cnfrmrId" />
        ...
        <div class="card">
          <div class="card-header">
            ...
            <dl class="sr-info">
              <dt>요청범주</dt>
              <dd>
                <form:select path="srvcRsponsVO.srvcRsponsClCode" class="control w-auto">
              ...
              <dt>요청구분</dt>
              <dd>
                <form:select id="trgetSrvcCode" path="srvcRsponsVO.trgetSrvcDetailCode" class="control w-auto" >
                  <form:option value="">선택하세요</form:option>
                  <form:options items="${trgetSrvcDetailCodeVOList}" itemLabel="cmmnCodeNm" itemValue="cmmnCode"/>
                </form:select>
              </dd>
            </dl>
          </div>
...
-->
              ...
              <dl>
                ...
                <dd>
                  <div class="input-group datetime">
                    <form:input path="srvcRsponsVO.rspons1stDtDateDisplay" id="rspons1stDtDateDisplay"  class="control inpdate" title="접수일" cssStyle="ime-mode:disabled;" placeholder="YYYY-MM-DD" readonly="true"/>
                    <form:input path="srvcRsponsVO.rspons1stDtTimeDisplay" id="rspons1stDtTimeDisplay" class="control inptime" title="요청시간" cssStyle="ime-mode:disabled;" placeholder="HH:MM" readonly="true"/>
                  </div>
                </dd>
              </dl>
              <dl>
            ...
            <div class="detailbox type2">
              <dl>
                <dt>변경분류</dt>
                <dd>
	                <form:select id="srvcRsponsBasisCode" path="srvcRsponsVO.srvcRsponsBasisCode" class="control w-auto" title="변경분류">
	                  <form:option value="">선택하세요</form:option>
	                  <form:options items="${srvcRsponsBasisCodeVOList}" itemLabel="cmmnCodeNm" itemValue="cmmnCode"/>
	                </form:select>
                </dd>
              </dl>
              <dl>
                <dt>변경유형</dt>
                <dd>
                  <form:select style="width:130px;" id="trgetSrvcCode" path="srvcRsponsVO.trgetSrvcCode" cssClass="control " title="변경유형">
                    <form:option value="">선택하세요</form:option>
              ...
              <dl>
                <dt>SR처리시간</dt>
                <dd>
                  <form:input type='number' min='1' path="srvcRsponsVO.processTerm" id="processTerm" class="control"  title="처리시간" />
                </dd>
              </dl>
              <dl>
                <dt>시간구분</dt>
                <dd>
                  <form:select id="processStdrCode" path="srvcRsponsVO.processStdrCode" cssClass="control w-auto" title="SR처리기준시간">
<!--                     
                    <form:option value="">선택하세요</form:option>
-->                    
