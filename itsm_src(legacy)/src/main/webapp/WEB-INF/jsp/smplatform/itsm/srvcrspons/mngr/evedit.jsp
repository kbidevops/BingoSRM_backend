...
*/
      ...
      if (requstAtchmnflFileCnt > 0 && rsponsAtchmnflFileCnt > 0) {
        ...
        frm.action = "<c:url value="${registerFlag == 'create' ? '/srvcrspons/mngr/updatesrev.do' : '/srvcrspons/mngr/updatesrev.do'}"/>";
        frm.submit();
      }
    }
    ...
    function fncRetrieveList() {
      ...
      document.listForm.action = "<c:url value='/srvcrspons/mngr/retrieveSrEvList.do'/>";
      </c:if>
      <c:if test="${!empty srvcRsponsFormVO.returnListMode}">
      document.listForm.action = "<c:url value='/srvcrspons/mngr/retrieveSrEvList.do.do'/>";
      </c:if>
      document.listForm.submit();
    }
...
<main id="content">
  ...
  <div class="page-header">
    <h3 class="page-title">
      <c:out value="${LEFT_MENU_PROGRMACCESAUTHORVO.progrmNm}"/> 
<!-- 
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="detailbox type2">
              ...
              <c:if test="${srvcRsponsFormVO.srvcRsponsVO.verifyUserNm != null }">
	              <dl>
	                  <dt>검증담당</dt>
	                  <dd><strong>${srvcRsponsFormVO.srvcRsponsVO.verifyUserNm }</strong></dd>
	              </dl>
	              <dl>
	                  <dt>완료담당</dt>
	                  <dd><strong>${srvcRsponsFormVO.srvcRsponsVO.finishUserNm }</strong></dd>
	              </dl>
              </c:if>
              <dl class="col-4">
                <dt>만족도</dt>
...
-->                
              ...
              <c:if test="${srvcRsponsFormVO.srvcRsponsVO.verifyDtDateDisplay != null }">
	              <dl>
	                <dt>검증일시</dt>
                      <dd><c:out value="${srvcRsponsFormVO.srvcRsponsVO.verifyDtDateDisplay}"/>  <c:out value="${srvcRsponsFormVO.srvcRsponsVO.verifyDtTimeDisplay}"/></dd>
	              </dl>
              </c:if>
              ...
              <c:if test="${srvcRsponsFormVO.srvcRsponsVO.finishDtDateDisplay != null }">
	              <dl>
	                <dt>완료일시</dt>
                      <dd><c:out value="${srvcRsponsFormVO.srvcRsponsVO.finishDtDateDisplay}"/>  <c:out value="${srvcRsponsFormVO.srvcRsponsVO.finishDtTimeDisplay}"/></dd>
	              </dl>
              </c:if>
...
<!--
              <dl>
                <dt>SR 구분</dt>
                <dd>
                  <form:select id="srvcRsponsClCode" path="srvcRsponsVO.srvcRsponsClCode" disabled="true" cssClass="control" title="SR 구분">
                    <form:option value="">선택하세요</form:option>
              ...
              <dl>
                <dt>SR처리기준시간</dt>
                <dd>
                  <form:select id="processStdrCode" path="srvcRsponsVO.processStdrCode" disabled="true" cssClass="control" title="SR처리기준시간">
                    <form:option value="">선택하세요</form:option>
...
-->              
            ...
            <c:if test="${srvcRsponsFormVO.srvcRsponsVO.verifyYn != 'Y' }">
	            <div class="detailbox type2">
	                <dl>
	                    <dt>검증내역</dt>
	                    <dd>
	                    	<div class="scrollbox control type2">
	                    		${fn:replace(srvcRsponsFormVO.srvcRsponsVO.srvcVerifyDtls, newLineChar, "<br/>")}
	                        </div>
	                    </dd>
	                </dl>
	            </div>
	            <div class="detailbox type2">
	                <dl>
	                    ...
	                    <dd>
	                    	<div class="scrollbox control type2">
	                    		${fn:replace(srvcRsponsFormVO.srvcRsponsVO.srvcFinDtls, newLineChar, "<br/>")}
	                        </div>
	                    </dd>
	                </dl>
	            </div>
