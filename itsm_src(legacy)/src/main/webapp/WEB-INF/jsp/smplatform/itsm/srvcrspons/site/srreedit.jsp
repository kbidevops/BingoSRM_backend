...
<head>
  ...
  <!--For Commons Validator Client Side-->0
    ...
    $(document).ready(function() {
      ...
      $('#requstAtchmnfl').fileupload({
        ...
        progressall: function (e, data) {
          ...
          if (progress === 100) {
            frm.action = "<c:url value="${registerFlag == 'create' ? '/srvcrspons/site/createsr.do' : '/srvcrspons/site/updatesr.do'}"/>";
            frm.submit();
          }
        },
...
*/
      ...
      if (requstAtchmnflFileCnt > 0) {
        ...
        frm.action = "<c:url value="${registerFlag == 'create' ? '/srvcrspons/site/createsrre.do' : '/srvcrspons/site/createsrre.do'}"/>";
        frm.submit();
      }
    }
    ...
    function fncDelete() {
      confirm('삭제하시겠습니까?', 'SR요청 삭제', null, function(request) {
        ...
        if (request) {
          document.listForm.action = "<c:url value='/srvcrspons/site/deletesr.do'/>";
...
-->      
  ...
  <form:form commandName="srvcRsponsFormVO" id="listForm" name="listForm" method="post" enctype="multipart/form-data">
    ...
    <form:hidden path="srvcRsponsVO.reSrvcRsponsNo" />
    <form:hidden path="srvcRsponsVO.reSrvcRsponsCn" />
    <form:hidden path="srvcRsponsVO.reSrvcRsponsSj" />
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="detailbox type1">
                  ...
                  <dl>
                    <dt>참조 아이디</dt>
                    <dd><form:input path="srvcRsponsVO.refIds" cssClass="control" tabindex="4" cssStyle="ime-mode:inactive;" title="참조 아이디"/></dd>
                  </dl>
                </div>
              </div>
            </div>
                ...
                <div class="detailbox type1">
                  ...
                  <dl>
                    <c:if test="${!empty srvcRsponsFormVO.srvcRsponsVO.reSrvcRsponsSj}">
                		<dt>1차 SR요청 정보</dt>
                		<dd>
                		<textarea path="srvcRsponsVO.reSrvcRsponsCn" readonly cssStyle="ime-mode:active;" class="control" title="요청상세내용" rows="10"/>
요청제목 : ${srvcRsponsFormVO.srvcRsponsVO.reSrvcRsponsSj}
요청상세내용 :
${fn:replace(srvcRsponsFormVO.srvcRsponsVO.reSrvcRsponsCn, newLineChar, "<br/>")}	
                		</textarea>
