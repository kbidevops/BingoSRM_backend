	          		...
	          		<c:if test="${result.orderLevel == '3' }">
	          		  <li>
	                        ...
	                        <div class="btnbox center">
	                            <button type="button" onclick="fncUpdateStateView('<c:out value="${result.srvcRsponsNo}"/>')" class="btn btn-soft">상세보기</button>
	          					<c:if test="${result.orderLevel == '1' }">
								<button type="button" onclick="fncUpdateStateAjax('<c:out value="${result.srvcRsponsNo}"/>')" class="btn btn-soft">1차 응답</button>
	                            </c:if>
	                        </div>
	                        <c:if test="${result.orderLevel == '1' }">
