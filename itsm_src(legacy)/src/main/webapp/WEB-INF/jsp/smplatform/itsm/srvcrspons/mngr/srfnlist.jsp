...
<%
  /**
  * @Class Name : /srvcrspons/mngr/list.jsp
	...
	<script type="text/javaScript" language="javascript" defer="defer">
        ...
        function fncUpdateView(srvcRsponsNo) {
        	document.listForm.elements['srvcRsponsVO.srvcRsponsNo'].value = srvcRsponsNo;
           	document.listForm.action = "<c:url value='/srvcrspons/mngr/updateSrFnView.do'/>";
        ...
        function fncRetrieveList() {
        	document.listForm.action = "<c:url value='/srvcrspons/mngr/retrieveSrFnList.do'/>";
...
//         	console.log('processMt: '+processMt);
        ...
        function fncRetrievePagingList(pageNo){
        	...
        	document.listForm.action = "<c:url value='/srvcrspons/mngr/retrieveEvList.do'/>";
           	document.listForm.submit();
        }
        ...
        function fncReset(){
        	...
        	//document.listForm.elements['searchSrvcRsponsVO.chargerId'].value = '';
        	document.listForm.elements['searchSrvcRsponsVO.srcRqesterId'].value = '';
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="detailbox type1">
                                    ...
                                    <c:if test="${srvcRsponsFormVO.srvcRsponsVO.userTyCode != 'R005' }">
	                                    <dl>
	                                        <dt>요청자ID/이름</dt>
	                                        <dd><form:input path="searchSrvcRsponsVO.srcRqesterId"  class="control" title="요청자ID"/></dd>
	                                    </dl>
									</c:if>
<!-- 
                                    <dl>
                                        <dt>처리담당</dt>
                                        <dd>
                                          <form:select path="searchSrvcRsponsVO.chargerId" cssClass="control" title="접수자">
										  	<form:option value="">전체</form:option>
										  	<form:options items="${srvcManagerCodeVOList}" itemLabel="userNm" itemValue="userId"/>
										  </form:select>
                                        </dd>
                                    </dl>
-->
                                ...
                                <tbody>
                                  <c:forEach var="result" items="${resultList}" varStatus="status">
                                    <tr>
                                        ...
                                        <c:if test="${result.priorLevel == '0' }">
	                                        <td><a style="color: red;" href="javascript:fncUpdateView('<c:out value="${result.srvcRsponsNo}"/>')" class="ellipsis"><c:out value="${result.srvcRsponsSj}"/></a></td>
										</c:if>
                                        <c:if test="${result.priorLevel == '1' }">
                                        ...
                                        <td><c:out value="${result.trgetSrvcDetailCodeNm}"/></td>
                                        ...
                                        <c:choose>
                                        	<c:when test="${!empty result.reSrvcRsponsNo}"> 
												<td class="num"><c:out value="${result.reSrvcRsponsNo}"/></td>
											</c:when> 
											<c:otherwise> 
